...
- **Status:** Live. Gated app on Streamlit Community Cloud (tabs: Search · Topic Cloud · Manual Preview · About); corpus fetched privately at startup. `indexer.py` + `app.py` + `cloud.py` done and validated (golden set passes; AppTest green). Remaining: promote the inline self-checks into a `pytest` suite.
