...
- **Environment:** uv-managed venv, Python 3.12 (`.python-version`). `uv sync` to set up; run with `uv run python scripts/indexer.py` / `uv run streamlit run scripts/app.py` (port 8503 via `.streamlit/config.toml`). `pyproject.toml` + `uv.lock` are the source of truth; `requirements.txt` is the deploy mirror (kept in sync).
